    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--foreground)] mb-1">Campaign Explorer</h1>
                ...
                <div className="flex gap-2">
                    <div className="relative group">
                        ...
                        <input type="text" className="bg-[var(--surface-highlight)] border border-[var(--border)] rounded-md pl-9 pr-4 py-2 text-sm text-[var(--foreground)] w-64 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-all" placeholder="Search campaigns, hashtags..." />
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {campaigns.map((camp) => (
                    <Link href={`/dashboard/campaigns/${camp.id}`} key={camp.id}>
                        <Card className="hover:border-primary/50 transition-all duration-300 cursor-pointer group h-full hover:-translate-y-1 hover:shadow-[0_5px_20px_rgba(0,0,0,0.5)]">
                            <div className="flex justify-between items-start mb-4">
                                <div className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${camp.risk > 80 ? 'bg-red-900/40 text-[var(--accent)] border border-red-500/30' :
                                    camp.risk > 50 ? 'bg-yellow-900/40 text-yellow-500 border border-yellow-500/30' :
                                        'bg-green-900/40 text-[var(--primary)] border border-green-500/30'
                                    }`}>
                            ...
                            <h3 className="text-lg font-bold text-[var(--foreground)] mb-2 group-hover:text-primary transition-colors line-clamp-1">{camp.name}</h3>
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-400 mb-6">
                                ...
                                <span className={camp.status === 'Active' ? 'text-[var(--foreground)] font-medium flex items-center gap-2' : ''}>
                            ...
                            <div className="flex items-center justify-between mt-auto pt-4 border-t border-[var(--border)]">
                                <div className="flex -space-x-2">
                                    {camp.platforms.map((p, i) => (
                                        <div key={i} className="w-8 h-8 rounded-full bg-[var(--surface-highlight)] border border-[var(--border)] flex items-center justify-center text-gray-400 z-0 text-xs">
                                            ...
                                            {p === 'tiktok' && <span className="font-bold">TT</span>}
                                            {p === 'youtube' && <span className="font-bold">YT</span>}
                                            {p === 'reddit' && <span className="font-bold">RD</span>}
                                            {p === 'whatsapp' && <span className="font-bold">WA</span>}
                                        </div>
                                    ))}
                                </div>
